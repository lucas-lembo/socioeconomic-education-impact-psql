-- This script was generated by the ERD tool in pgAdmin 4.
-- Please log an issue at https://github.com/pgadmin-org/pgadmin4/issues/new/choose if you find any bugs, including reproduction steps.
BEGIN;


CREATE TABLE IF NOT EXISTS public."AprovacaoSerie"
(
    "idAprovacaoSerie" serial NOT NULL,
    serie integer NOT NULL,
    porcentagem double precision,
    "idTaxasAprovacao" serial NOT NULL,
    CONSTRAINT "AprovacaoSerie_pkey" PRIMARY KEY ("idAprovacaoSerie")
);
    
CREATE TABLE IF NOT EXISTS public."ClassificacaoSocioEconomica"
(
    "idClassificacaoSE" integer NOT NULL,
    "qtdQuartos" integer NOT NULL,
    "qtdCelulares" integer NOT NULL,
    "qtdGeladeiras" integer NOT NULL,
    "qtdTelevisores" integer NOT NULL,
    "qtdBanheiros" integer NOT NULL,
    "idEscolaridadePais" integer NOT NULL,
    "qtdCarros" integer NOT NULL,
    "qtdComputadores" integer NOT NULL,
    CONSTRAINT "ClassificacaoSocioEconomica_pkey" PRIMARY KEY ("idClassificacaoSE")
);

CREATE TABLE IF NOT EXISTS public."Escola"
(
    "idEscola" character varying COLLATE pg_catalog."default" NOT NULL,
    "idMunicipio" character varying COLLATE pg_catalog."default" NOT NULL,
    nome character varying COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT "Escola_pkey" PRIMARY KEY ("idEscola")
);

CREATE TABLE IF NOT EXISTS public."Estado"
(
    uf character varying COLLATE pg_catalog."default" NOT NULL,
    nome character varying COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT "Estado_pkey" PRIMARY KEY (uf)
);

CREATE TABLE IF NOT EXISTS public."IndicadoresAlunos"
(
    "idIndicadoresAlunos" serial NOT NULL,
    "qtdAlunosInse" integer NOT NULL,
    "anoMedicao" integer NOT NULL,
    "idEscola" character varying COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT "IndicadoresAlunos_pkey" PRIMARY KEY ("idIndicadoresAlunos")
);

CREATE TABLE IF NOT EXISTS public."Municipio"
(
    "idMunicipio" character varying COLLATE pg_catalog."default" NOT NULL,
    uf character varying COLLATE pg_catalog."default" NOT NULL,
    nome character varying COLLATE pg_catalog."default" NOT NULL,
    "idTipoCapital" integer NOT NULL,
    CONSTRAINT "Municipio_pkey" PRIMARY KEY ("idMunicipio")
);

CREATE TABLE IF NOT EXISTS public."NivelEscolaridade"
(
    "idEscolaridadePais" integer NOT NULL,
    escolaridade character varying COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT "NivelEscolaridade_pkey" PRIMARY KEY ("idEscolaridadePais")
);

CREATE TABLE IF NOT EXISTS public."NotaIDEB"
(
    "idIdeb" serial NOT NULL,
    "idEscola" character varying COLLATE pg_catalog."default" NOT NULL,
    "notaIdeb" double precision,
    "anoMedicao" integer NOT NULL,
    CONSTRAINT "NotaIDEB_pkey" PRIMARY KEY ("idIdeb")
);

CREATE TABLE IF NOT EXISTS public."NotaSAEB"
(
    "idSaeb" serial NOT NULL,
    "idEscola" character varying COLLATE pg_catalog."default" NOT NULL,
    "notaMatematica" double precision,
    "notaLinguaPort" double precision,
    "notaPadronizada" double precision,
    "anoMedicao" integer NOT NULL,
    CONSTRAINT "NotaSAEB_pkey" PRIMARY KEY ("idSaeb")
);

CREATE TABLE IF NOT EXISTS public."PorcentagemClassificacaoAlunos"
(
    "idPorcentagem" serial NOT NULL,
    "idIndicadoresAlunos" serial NOT NULL,
    "idClassificacaoSE" integer NOT NULL,
    "porcentagemAlunos" double precision NOT NULL,
    CONSTRAINT "PorcentagemClassificacaoAlunos_pkey" PRIMARY KEY ("idPorcentagem")
);

CREATE TABLE IF NOT EXISTS public."TaxasAprovacao"
(
    "idTaxasAprovacao" serial NOT NULL,
    "idEscola" character varying COLLATE pg_catalog."default" NOT NULL,
    "anoMedicao" integer NOT NULL,
    total double precision,
    CONSTRAINT "TaxasAprovacao_pkey" PRIMARY KEY ("idTaxasAprovacao")
);

CREATE TABLE IF NOT EXISTS public."TipoCapital"
(
    "idTipoCapital" integer NOT NULL,
    nome character varying COLLATE pg_catalog."default" NOT NULL,
    CONSTRAINT "TipoCapital_pkey" PRIMARY KEY ("idTipoCapital")
);

ALTER TABLE IF EXISTS public."AprovacaoSerie"
    ADD CONSTRAINT "idTaxasAprovacao" FOREIGN KEY ("idTaxasAprovacao")
    REFERENCES public."TaxasAprovacao" ("idTaxasAprovacao") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."ClassificacaoSocioEconomica"
    ADD CONSTRAINT "idEscolaridadePais" FOREIGN KEY ("idEscolaridadePais")
    REFERENCES public."NivelEscolaridade" ("idEscolaridadePais") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."Escola"
    ADD CONSTRAINT "idMunicipio" FOREIGN KEY ("idMunicipio")
    REFERENCES public."Municipio" ("idMunicipio") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."IndicadoresAlunos"
    ADD CONSTRAINT "idEscola" FOREIGN KEY ("idEscola")
    REFERENCES public."Escola" ("idEscola") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."Municipio"
    ADD CONSTRAINT "Municipio_uf_fkey" FOREIGN KEY (uf)
    REFERENCES public."Estado" (uf) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."Municipio"
    ADD CONSTRAINT "idTipoCapital" FOREIGN KEY ("idTipoCapital")
    REFERENCES public."TipoCapital" ("idTipoCapital") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."NotaIDEB"
    ADD CONSTRAINT "idEscola" FOREIGN KEY ("idEscola")
    REFERENCES public."Escola" ("idEscola") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."NotaSAEB"
    ADD CONSTRAINT "idEscola" FOREIGN KEY ("idEscola")
    REFERENCES public."Escola" ("idEscola") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."PorcentagemClassificacaoAlunos"
    ADD CONSTRAINT "idClassificacaoSE" FOREIGN KEY ("idClassificacaoSE")
    REFERENCES public."ClassificacaoSocioEconomica" ("idClassificacaoSE") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."PorcentagemClassificacaoAlunos"
    ADD CONSTRAINT "idIndicadoresAlunos" FOREIGN KEY ("idIndicadoresAlunos")
    REFERENCES public."IndicadoresAlunos" ("idIndicadoresAlunos") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;


ALTER TABLE IF EXISTS public."TaxasAprovacao"
    ADD CONSTRAINT "idEscola" FOREIGN KEY ("idEscola")
    REFERENCES public."Escola" ("idEscola") MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID;

END;